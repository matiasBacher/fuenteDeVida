...
async function borrarProducto(codigo, nombre){
    ...
    if(mensaje==-1 || mensaje==0){
        alert("No se pudo Borrar el producto: "+nompreProducto)
        return;
    }
    if(mensaje==1){
        devolverBusquedaProducto(buscarElemento.value)
        alert("El producto: "+nombreProducto+" se ha borrado")
        return; 
    }
...
if (productosEnMemoria.length === 0) {
    ...
    celda.colSpan = 7;
    celda.textContent = 'Producto NO Encontrado';
    ...
    productosEnMemoria.forEach(function(x) {
        ...
        buttonModify.addEventListener("click", ()=>{
            modalModifPro.style.display="flex"})
        ...
        buttonRemove.addEventListener("click", ()=>{
            borrarProducto(x.codigo,x.nombre)
        })
